...
// Função de Estilo (Cores e Ícones)
function getEstiloEvento(nome: string, tipo: string) {
  const nomeUpper = nome.toUpperCase();
  const tipoUpper = tipo.toUpperCase();

  // 1. TGR (MOVIDO PARA O TOPO: Prioridade Alta)
  // Verifica antes de "ETAPA" para evitar conflito com "Etapa da TGR"
  ...
  if (nomeUpper.includes("INTERNO")) {
    return {
      border: "border-l-[var(--leao-vermelho)]",
  ...
  // 4. Estadual / Etapa Genérica (Verifica por último entre os locais)
  if (
...
export default async function CalendarioPage() {
  // 1. Busca APENAS do Banco de Dados
  const campeonatosDB = await prisma.campeonato.findMany({
    orderBy: { data: "asc" },
  });

  // 2. Mapeia para o formato do calendário
  ...
  todosEventos.forEach((evento) => {
    ...
    // Usa timeZone: "UTC" para não voltar 1 dia
          ...
          {Object.keys(eventosPorMes).map((mes) => (
            ...
            >
              ...
              <div className="sticky top-24 z-10 bg-[#0a0a0a]/95 backdrop-blur-sm py-4 border-b border-zinc-800 mb-6">
                <h2 className="text-3xl font-barlow font-black text-(--leao-vermelho) uppercase tracking-wide flex items-center gap-3">
              ...
              <div className="grid gap-4">
                {eventosPorMes[mes].map((evento) => {
                  ...
                  // Usar getUTCDate() para pegar o dia exato sem fuso
                  const dia = dataObj.getUTCDate();

                  // Usar timeZone: "UTC" para o dia da semana
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-col items-center justify-center min-w-20 text-center border-r border-zinc-800 md:border-0 pr-4 md:pr-0">
                        <span className="text-4xl font-barlow font-black text-(--leao-amarelo) leading-none group-hover:scale-110 transition-transform">
                          {dia}
                        </span>
                      ...
                      <div className="flex-1">
                        ...
                        <h3 className="text-xl md:text-2xl font-barlow font-bold text-(--leao-vermelho) uppercase group-hover:text-(--leao-amarelo) transition-colors">
                          {evento.nome}
                        </h3>
                      </div>
